-- Drop and recreate player_match_statistics with proper NULL handling
...
-- Create player_match_statistics table with proper NULL handling
CREATE TABLE player_match_statistics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  player_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  opponent_id UUID,  -- NULL = global stats
  map_id UUID,  -- NULL = all maps
  faction_id UUID,  -- NULL = all factions
  opponent_faction_id UUID,  -- NULL = all opponent factions
...
-- Foreign keys for nullable columns
ALTER TABLE player_match_statistics
ADD CONSTRAINT fk_opponent FOREIGN KEY (opponent_id) REFERENCES users(id) ON DELETE CASCADE,
ADD CONSTRAINT fk_map FOREIGN KEY (map_id) REFERENCES game_maps(id) ON DELETE CASCADE,
ADD CONSTRAINT fk_faction FOREIGN KEY (faction_id) REFERENCES factions(id) ON DELETE CASCADE,
ADD CONSTRAINT fk_opponent_faction FOREIGN KEY (opponent_faction_id) REFERENCES factions(id) ON DELETE CASCADE;
