-- Fix: Combine H2H winner and loser stats BEFORE inserting to avoid ON CONFLICT overwriting
-- This is the root cause of opponent stats showing only 1 match instead of aggregated totals
...
BEGIN
  ...
  -- ===== HEAD-TO-HEAD COMBINED (merge winner and loser results first) =====
  ...
  h2h_combined AS (
    SELECT
      ...
      MAX(last_elo_against_me) as last_elo_against_me,
      AVG(avg_elo_change) as avg_elo_change,
      SUM(elo_gained) as total_elo_gained,
      SUM(elo_lost) as total_elo_lost,
      MAX(last_match_date) as last_match_date
    FROM (
      SELECT player_id, opponent_id, map_id, faction_id, opponent_faction_id, wins, losses, last_elo_against_me, avg_elo_change, elo_gained, elo_lost, last_match_date FROM h2h_winner_agg
      UNION ALL
      SELECT player_id, opponent_id, map_id, faction_id, opponent_faction_id, wins, losses, last_elo_against_me, avg_elo_change, elo_gained, elo_lost, last_match_date FROM h2h_loser_agg
    ) combined
  ...
  SELECT
    ...
    total_wins,
    total_losses,
    CASE WHEN total_games > 0 THEN ROUND(100.0 * total_wins / total_games, 2)::NUMERIC(5,2) ELSE 0 END,
    avg_elo_change,
    last_elo_against_me,
    total_elo_gained,
    total_elo_lost,
