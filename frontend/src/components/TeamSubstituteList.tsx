...
import { TeamReplacementModal } from './TeamReplacementModal';
    ...
    <>
      <div className="bg-gray-50 border border-gray-300 rounded-md p-3 mb-4">
        <div className="flex items-center justify-between mb-3">
          <h4 className="m-0 text-sm font-semibold text-gray-800">{t('Substitutes')} ({t('Backup Players')})</h4>
          {isOrganizerView && teamMembers.length > 0 && (
            <button
              type="button"
        ...
        <div className="flex flex-col gap-2 mb-3">
          {substitutes.length === 0 ? (
            <p className="text-gray-500 italic text-sm m-0 p-2">{t('No substitutes yet')}</p>
          ) : (
            substitutes
        ...
        {isOrganizerView && (
          <div className="grid grid-cols-2 gap-2 md:grid-cols-1">
            ...
            >
              <option value="">-- {t('Select substitute player')} --</option>
            ...
            >
              {loading ? t('Adding...') : t('Add Substitute')}
            </button>
          </div>
        )}
      </div>
      ...
      <TeamReplacementModal
        ...
        onSuccess={onSubstitutesUpdated}
