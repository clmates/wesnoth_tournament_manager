...
const OpponentStats: React.FC<OpponentStatsProps> = ({ matches, currentPlayerId }) => {
  ...
  useEffect(() => {
    const fetchOpponents = async () => {
      try {
        setLoading(true);
        const data = await playerStatisticsService.getRecentOpponents(currentPlayerId, 100, side);
        setOpponents(data || []);
      } catch (err) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-6 my-6">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6 pb-4 border-b-2 border-gray-200">
        <h3 className="text-xl font-semibold text-gray-800">{t('my_opponents') || 'My Opponents'}</h3>
          ...
          <table className="w-full border-collapse text-sm bg-white">
            <thead>
              <tr className="bg-gradient-to-r from-gray-700 to-gray-800 text-white">
                <th className="px-3 py-4 text-left font-semibold text-xs uppercase tracking-wider border-b-2 border-gray-900 cursor-pointer hover:bg-gray-600 transition-colors select-none" onClick={() => handleSort('opponent_name')}>{t('opponent_name') || 'Opponent'}{getSortIndicator('opponent_name')}</th>
                <th className="px-3 py-4 text-center font-semibold text-xs uppercase tracking-wider border-b-2 border-gray-900 cursor-pointer hover:bg-gray-600 transition-colors select-none" onClick={() => handleSort('total_games')}>{t('games') || 'Games'}{getSortIndicator('total_games')}</th>
                <th className="px-3 py-4 text-center font-semibold text-xs uppercase tracking-wider border-b-2 border-gray-900 cursor-pointer hover:bg-gray-600 transition-colors select-none" onClick={() => handleSort('wins')}>{t('wins') || 'Wins'}{getSortIndicator('wins')}</th>
                <th className="px-3 py-4 text-center font-semibold text-xs uppercase tracking-wider border-b-2 border-gray-900 cursor-pointer hover:bg-gray-600 transition-colors select-none" onClick={() => handleSort('losses')}>{t('losses') || 'Losses'}{getSortIndicator('losses')}</th>
                <th className="px-3 py-4 text-center font-semibold text-xs uppercase tracking-wider border-b-2 border-gray-900 cursor-pointer hover:bg-gray-600 transition-colors select-none" onClick={() => handleSort('winrate')}>{t('winrate') || 'W/R %'}{getSortIndicator('winrate')}</th>
                <th className="px-3 py-4 text-center font-semibold text-xs uppercase tracking-wider border-b-2 border-gray-900 cursor-pointer hover:bg-gray-600 transition-colors select-none" onClick={() => handleSort('current_elo')}>{t('current_elo') || 'Current ELO'}{getSortIndicator('current_elo')}</th>
                <th className="px-3 py-4 text-center font-semibold text-xs uppercase tracking-wider border-b-2 border-gray-900 cursor-pointer hover:bg-gray-600 transition-colors select-none" onClick={() => handleSort('elo_gained')}>ELO Gained{getSortIndicator('elo_gained')}</th>
                <th className="px-3 py-4 text-center font-semibold text-xs uppercase tracking-wider border-b-2 border-gray-900 cursor-pointer hover:bg-gray-600 transition-colors select-none" onClick={() => handleSort('elo_lost')}>ELO Lost{getSortIndicator('elo_lost')}</th>
                <th className="px-3 py-4 text-center font-semibold text-xs uppercase tracking-wider border-b-2 border-gray-900 cursor-pointer hover:bg-gray-600 transition-colors select-none" onClick={() => handleSort('last_elo_against_me')}>Last ELO vs Me{getSortIndicator('last_elo_against_me')}</th>
                <th className="px-3 py-4 text-left font-semibold text-xs uppercase tracking-wider border-b-2 border-gray-900 cursor-pointer hover:bg-gray-600 transition-colors select-none" onClick={() => handleSort('last_match_date')}>{t('last_match') || 'Last Match'}{getSortIndicator('last_match_date')}</th>
              </tr>
            </thead>
            <tbody>
              {getSortedOpponents().map((opponent, index) => {
                ...
                return (
                  <tr key={`${opponent.opponent_id}-${opponent.player_side}`} className={`border-b border-gray-200 transition-colors hover:bg-blue-50 ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}>
                    ...
                    <td className="px-3 py-3 text-center font-medium text-gray-600"><strong>{opponent.total_games}</strong></td>
                    ...
                    <td className="px-3 py-3 text-center align-middle">
                      <span className={`inline-block px-2.5 py-1 rounded font-semibold text-xs shadow-sm ${
                        winrate > 55 ? 'bg-gradient-to-br from-green-100 to-green-200 text-green-900'
                          : winrate < 45 ? 'bg-gradient-to-br from-red-100 to-red-200 text-red-900'
                    ...
                    <td className="px-3 py-3 text-center font-bold text-green-600">+{eloGained.toFixed(2)}</td>
                    <td className="px-3 py-3 text-center font-bold text-red-600">-{eloLost.toFixed(2)}</td>
                    <td className="px-3 py-3 text-center text-gray-700 font-semibold">{lastEloAgainstMe.toFixed(0)}</td>
